...
COMMON_SRC = src/sysinfo.cpp src/jokes.cpp src/ascii_art.cpp src/display.cpp src/locales.cpp src/palettes.cpp src/config.cpp src/tui_config.cpp src/fatjump.cpp src/discord_rpc.cpp src/fatgotchi.cpp src/fatrain.cpp
...
all: fatfetch fatjump fatrpc fatgotchi fatrain fatfetch-installer
...
install: fatfetch fatjump fatrpc fatgotchi fatrain
	...
	chmod 755 $(DESTDIR)$(PREFIX)/bin/fatfetch $(DESTDIR)$(PREFIX)/bin/fatjump $(DESTDIR)$(PREFIX)/bin/fatrpc $(DESTDIR)$(PREFIX)/bin/fatgotchi $(DESTDIR)$(PREFIX)/bin/fatrain
	@echo "FATfetch, FATjump, FATrpc, FATgotchi and FATrain installed to $(DESTDIR)$(PREFIX)/bin"

uninstall:
	rm -f $(DESTDIR)$(PREFIX)/bin/fatfetch $(DESTDIR)$(PREFIX)/bin/fatjump $(DESTDIR)$(PREFIX)/bin/fatrpc $(DESTDIR)$(PREFIX)/bin/fatgotchi $(DESTDIR)$(PREFIX)/bin/fatrain
	@echo "Uninstalled from $(DESTDIR)$(PREFIX)/bin"

zip: all
	chmod +x install.sh fatfetch fatjump fatrpc fatgotchi fatrain fatfetch-installer
	zip -r $(DIST_ZIP) fatfetch fatjump fatrpc fatgotchi fatrain fatfetch-installer install.sh README.md Makefile PKGBUILD LICENSE src/
	@echo "Created $(DIST_ZIP) successfully!"

clean:
	rm -f fatfetch fatjump fatrpc fatgotchi fatrain fatfetch-installer $(DIST_ZIP)
